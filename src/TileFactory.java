...
public class TileFactory {
    ...
    public Map<Character,Enemy> mapEnemies() {
        return enemiesMap.values().stream().map(Supplier::get).collect(Collectors.toMap(Enemy::getTile, Function.identity()));
    }
    ...
    public Enemy produceEnemy(char tile, Position position){
        ...
        if (enemySupplier == null) {
            throw new IllegalArgumentException("No enemy type for tile: " + tile);
        }
    ...
    public Wall produceWall(Position position){ //maybe more identifiers
